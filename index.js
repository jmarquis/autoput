...
if (process.argv.length >= 3 && process.argv[2] === "init") {
	...
	fs.readFile(__dirname + "/sftp.json", function (error, data) {
		if (error) {
			console.error(error);
			process.exit(1);
		} else {
			fs.writeFile(process.cwd() + "/sftp.json", data, {
				flag: "wx"
			}, function (error) {
				if (error) {
					if (error.code === "EEXIST") {
						console.log("sftp.json already exists!");
					} else {
						console.error(error);
						process.exit(1);
					}
				} else {
					console.log("sftp.json created");
				}
			});
		}
	});
	...
	var _ = {
		...
		init: function () {
			...
			if (!config) {
				console.log("sftp.json is not present, run autoput init to create");
			}
